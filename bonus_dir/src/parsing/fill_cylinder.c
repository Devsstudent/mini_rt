...
/*   Updated: 2023/02/07 00:33:36 by odessein         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "parsing.h"

bool	fill_cylinder_1(char **arr, int j, t_objects *objects)
{
	...
	if (!convert_to_rgb(&objects->cy[j].color, arr[5]))
		return (false);
	if (BONUS && arr[6] && !check_texture(&objects->cy[j].tex, arr[6], objects))
		return (false);
	else if (!BONUS && arr[6])
		return (false);
	return (true);
...
{
	if (!fill_cylinder_1(arr, j, objects))
		return (false);
