...
/*   Updated: 2022/12/02 17:27:33 by odessein         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minirt.h"
...
bool	check_sphere(char **line_split, t_must_have *all_elem)
{
	...
	first = line_split[0] && (line_split[0][0] == 's' || line_split[0][0] == 'S') && line_split[0][1] && (line_split[0][1] == 'p' || line_split[0][1] == 'P') && !line_split[0][2];
	if (!first)
		return (false);
