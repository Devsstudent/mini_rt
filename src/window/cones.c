...
bool	get_quadra_cones_equation(t_line_eq rayline, t_cones cones)
{
	t_equation	res;
	t_xyz		origin;
	float		height;
...
bool	get_cones(t_objects *obj, t_sol_li *list, t_line_eq rayline)
{
	...
	while (i < obj->nb_co)
	{
		...
		solu = solu(quadra, rayline, &err);
		if (err)
			return (false);
		if (solu.one && !list_add(list, new_elem(solu, obj->co[i].color, CO, i)))
			return (false);
		i++;
	}
}
...
bool	get_specific_cone(t_objects *obj, t_sol_li *list, t_line_eq rayline, int i_to_view)
{
	...
	err = false;
	quadratic = get_quadra_cones_equation(rayline, obj->co[i_to_view])l
	solu = solution(quadratic, rayline, &err);
	if (err)
		return (false);
	if (solu.sol_one && !list_add(list ,new_elem(solu, obj->co[i_to_view].color, CO, i)))
		return (false);
	return (true);
}
