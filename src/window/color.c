...
/*   Updated: 2023/01/02 18:01:03 by mbelrhaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minirt.h"
...
int	create_color(t_rgb rgb, float RGB[3])
{
	...
	color_conv.rgb.G = (uint8_t)trunc((float)color_conv.rgb.G * RGB[1]);
	color_conv.rgb.B = (uint8_t)trunc((float)color_conv.rgb.B * RGB[2]);
...
bool	add_light(t_disp_point disp_p, t_objects *objs, float RGB[3])
{
	...
	while (i < objs->nb_li)
	{
		...
		if (intersection.intersec_point.x != -1 && intersection.intersec_point.y != -1 && intersection.intersec_point.z != -1 && in_the_way(intersection.intersec_point, rayvec, disp_p.intersec_point))
		{
			i++;
			continue ;
		}
