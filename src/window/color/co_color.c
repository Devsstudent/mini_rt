...
/*   Updated: 2023/01/31 21:24:37 by odessein         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "window.h"

static float	get_teta(t_vect dist, t_cone co, t_disp_point disp_p,
					t_vect *len)
{
	float	teta;
	float	y;
	...
	*len = (create_vector(b, disp_p.intersec_point));
	teta = acos(scalar_product(*len, co.vec_width)
			/ norm_of_vector(*len)) * 180 / M_PI;
	if (z < 0)
		teta = 360 - teta;
	return (teta);
}
...
static void	co_checkerboard(t_rgb *color, t_disp_point disp_p, t_cone co)
{
	...
	if ((int)(teta / 30) % 2 == 0)
	{
		if ((int)norm_of_vector(dist) / (int)(30 * M_PI
			/ 180 * co.radius) % 2 == 0)
