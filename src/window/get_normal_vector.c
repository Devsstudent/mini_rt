...
/*   Updated: 2023/01/16 19:19:53 by mbelrhaz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "window.h"
...
t_vect	get_normal_vect_cy(t_disp_point intersec, t_objects *objs)
{
	...
	normal_cy[0] = intersec.intersec_point.x
		- (a * t + objs->cy[intersec.obj_id].position.x);
	normal_cy[1] = intersec.intersec_point.y
		- (b * t + objs->cy[intersec.obj_id].position.y);
	normal_cy[2] = intersec.intersec_point.z
		- (c * t + objs->cy[intersec.obj_id].position.z);
	return (normal_cy);
}
