...
static bool init_mlx(t_objects *objects)
{
	...
	objects->mlx->win = mlx_new_window(objects->mlx->mlx, 780, 540, "mini_rt :)");
	...
	objects->mlx->data = mlx_get_data_addr(objects->mlx->img, &objects->mlx->bpp,
			&objects->mlx->line_size, &objects->mlx->endian);
	if (!objects->mlx->data)
		return (close_window(objects), false);
	return (true);
